...
const [isModalOpen, setIsModalOpen] = useState(false);
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen ">
      ...
      <div className="grid gap-4">
        ...
        setSelectedJob(job);
        setIsModalOpen(true);
      }} className="border p-2 px-6 mr-10 rounded bg-blue-500 text-white">Details</button>
            </div>
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
    <div className="bg-white p-6 rounded-lg max-w-lg w-full relative">
      ...
      <h2 className="text-2xl font-bold text-blue-700 mb-2">{selectedJob.title}</h2>
      ...
      <p className="text-gray-600 mb-1"><strong>Job Type:</strong> {selectedJob.jobType}</p>
      <p className="text-gray-600 mb-1"><strong>Salary:</strong> {selectedJob.salary}</p>
      <p className="text-gray-600 mb-2"><strong>Skills:</strong> {selectedJob.skillsRequired?.join(', ')}</p>
      <p className="text-gray-800 whitespace-pre-line">{selectedJob.description}</p>
      <button className="border p-1 bg-green-500 text-white rounded">Apply</button>
    </div>
  </div>
)}
